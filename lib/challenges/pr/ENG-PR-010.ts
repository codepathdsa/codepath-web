...
const challenge: Challenge = {
  ...
  level: 'Junior',
  status: 'Not Started',
  desc: 'A security researcher privately reported they can download any user\'s invoice by changing a number in the URL. A junior dev wrote this endpoint last sprint. It passed code review — but the bug is sitting in plain sight.',
  ...
  prReview: {
    ...
    testing: 'Tested fetching own invoices — works correctly. Auth middleware rejects unauthenticated requests.',
    hints: [
      'The route checks that the user is authenticated — but does it check that this invoice belongs to this user?',
    ...
    correctBugType: 'idor',
    successExplanation: "Correct — this is a textbook IDOR (Insecure Direct Object Reference). The endpoint is authenticated (requireAuth), so anonymous users are blocked. But any logged-in user can request /invoices/1 through /invoices/N and retrieve every invoice in the database, because the query only filters on id — never on the requesting user's identity. Fix: AND user_id = $2 with req.user.id as the second parameter. Return 404 (not 403) so you don't leak whether an ID exists.",
